...
def generate_nate_calendar() -> dict[str, list[str]]:
    calendar = {}
    today = date.today()
    possible_times = [f"{h:02}:00" for h in range(9, 18)]  # 9 AM to 5 PM
    ...
    print("Nate's calendar:", calendar)
    return calendar

NATE_CALENDAR = generate_nate_calendar()
...
def get_availability(start_date: str, end_date: str) -> str:
    try:
        ...
        for i in range(delta.days + 1):
            day = start + timedelta(days=i)
            date_str = day.strftime("%Y-%m-%d")
            slots = NATE_CALENDAR.get(date_str, [])
            if slots:
                results.append(f"On {date_str}, Nate is available at: {', '.join(slots)}.")
            else:
                results.append(f"Nate is not available on {date_str}.")
...
def create_agent() -> LlmAgent:
    return LlmAgent(
        ...
        name="Nate_Agent",
        instruction="""
            **Role:** You are Nate's assistant for managing his calendar.
            **Directive:** Use the get_availability tool to check Nate's availability.
            Respond politely and stay on-topic about scheduling only.
        """,
